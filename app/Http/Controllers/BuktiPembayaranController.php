...
class BuktiPembayaranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //return response
        return redirect('/')->with('success', 'Bukti Pembayaran Berhasil Dikirim');
    }
    ...
    public function validasi_pembayaran(Request $request)
    {
        ...
        Keuangan::create([
            ...
            'saldo'        => '0',
            // 'saldo'         => Keuangan::orderBy('id_keuangan', 'desc')->first()->saldo + $pembayaran->uang_bayar,
        ]);
    ...
    public function edit($id_bukti_pembayaran)
    {
        $bukti=BuktiPembayaran::where('id_bukti_pembayaran', $id_bukti_pembayaran)->first();
        $pesanan= BuktiPembayaran::join('transaksi', 'bukti_pembayaran.id_transaksi', '=', 'transaksi.id_transaksi')
        ->join('pesanan', 'transaksi.id_pesanan', '=', 'pesanan.id_pesanan')
                    ->join('paket', 'pesanan.id_paket', '=', 'paket.id_paket')
                    ->join('pesanan_sistem', 'pesanan.id_pesanan', '=', 'pesanan_sistem.id_pesanan')
                    ->join('users', 'pesanan_sistem.id_pelanggan', '=', 'users.id')
                    ->join('jenis_paket', 'paket.id_jenis_paket', '=', 'jenis_paket.id_jenis_paket')
        ->where('bukti_pembayaran.id_bukti_pembayaran', $id_bukti_pembayaran)
        ->first();
        //dd ($pesanan);
        return view('admin.bukti_pembayaran.edit_pesanan', ['bukti' => $bukti, 'pesanan' => $pesanan]);
    }
    ...
    public function update(Request $request)
    {
        ...
        //return response
        return redirect('/admin/bukti-pembayaran')->with('success','Nominal Berhasil Diubah');
    }
}